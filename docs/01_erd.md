...
*   `status` (VARCHAR(20)): Каноническое строковое состояние сделки. Поддерживает следующие значения (соответствующие UI и EscrowDeal):
    *   `created` (соответствует UI-статусу: `создана` / Шаг 1)
    *   `active` или `hold` (соответствует UI-статусу: `оплачена` (Заморожено) / Шаг 2)
    *   `shipped` (соответствует UI-статусу: `отправлена` / Шаг 3)
    *   `completed` (соответствует UI-статусу: `завершена` / Шаг 4)
    *   `dispute` (соответствует UI-статусу: `спор` (Арбитраж))
*   `step` (INTEGER): Текущий численный шаг сделки (1: Создана, 2: Оплачена, 3: Отправлена, 4: Завершена).
*   `created_at` (TIMESTAMP).
